...
def build_vision_parser(subparsers: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    ...
    vision_parser = add_documented_parser(
        ...
        help_content=HelpContent(
            ...
            description="Create and maintain high-level containers composed of one or more task trees.",
            examples=(
                'lifeos vision add "Launch lifeos-cli" --area-id 11111111-1111-1111-1111-111111111111',
                "lifeos vision list --status active",
            ),
        ),
    )
    vision_parser.set_defaults(handler=make_help_handler(vision_parser))
    vision_subparsers = vision_parser.add_subparsers(dest="vision_command", title="actions", metavar="action")

    add_parser = add_documented_parser(vision_subparsers, "add", help_content=HelpContent(summary="Create a vision", description="Create a new vision."))
    ...
    add_parser.add_argument("--experience-rate-per-hour", type=int, help="Optional experience rate")
    add_parser.set_defaults(handler=handle_vision_add)

    list_parser = add_documented_parser(vision_subparsers, "list", help_content=HelpContent(summary="List visions", description="List visions with optional status or area filters."))
    list_parser.add_argument("--status", help="Filter by status")
    list_parser.add_argument("--area-id", type=UUID, help="Filter by area identifier")
    list_parser.add_argument("--include-deleted", action="store_true", help="Include soft-deleted visions")
    ...
    show_parser = add_documented_parser(vision_subparsers, "show", help_content=HelpContent(summary="Show a vision", description="Show one vision with full metadata."))
    show_parser.add_argument("vision_id", type=UUID, help="Vision identifier")
    show_parser.add_argument("--include-deleted", action="store_true", help="Allow deleted visions")
    show_parser.set_defaults(handler=handle_vision_show)

    update_parser = add_documented_parser(vision_subparsers, "update", help_content=HelpContent(summary="Update a vision", description="Update mutable vision fields."))
    ...
    update_parser.add_argument("--experience-rate-per-hour", type=int, help="Updated experience rate")
    update_parser.set_defaults(handler=handle_vision_update)

    delete_parser = add_documented_parser(vision_subparsers, "delete", help_content=HelpContent(summary="Delete a vision", description="Soft-delete a vision by default. Use --hard for permanent deletion."))
    delete_parser.add_argument("vision_id", type=UUID, help="Vision identifier")
    delete_parser.add_argument("--hard", action="store_true", help="Permanently delete the vision")
