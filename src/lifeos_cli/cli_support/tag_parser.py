...
def build_tag_parser(subparsers: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    ...
    tag_subparsers = tag_parser.add_subparsers(dest="tag_command", title="actions", metavar="action")

    add_parser = add_documented_parser(tag_subparsers, "add", help_content=HelpContent(summary="Create a tag", description="Create a new tag."))
    ...
    list_parser = add_documented_parser(tag_subparsers, "list", help_content=HelpContent(summary="List tags", description="List tags with optional filters."))
    list_parser.add_argument("--entity-type", help="Filter by entity type")
    list_parser.add_argument("--category", help="Filter by category")
    list_parser.add_argument("--include-deleted", action="store_true", help="Include soft-deleted tags")
    ...
    show_parser = add_documented_parser(tag_subparsers, "show", help_content=HelpContent(summary="Show a tag", description="Show one tag with full metadata."))
    show_parser.add_argument("tag_id", type=UUID, help="Tag identifier")
    show_parser.add_argument("--include-deleted", action="store_true", help="Allow deleted tags")
    show_parser.set_defaults(handler=handle_tag_show)

    update_parser = add_documented_parser(tag_subparsers, "update", help_content=HelpContent(summary="Update a tag", description="Update mutable tag fields."))
    ...
    delete_parser = add_documented_parser(tag_subparsers, "delete", help_content=HelpContent(summary="Delete a tag", description="Soft-delete a tag by default. Use --hard for permanent deletion."))
